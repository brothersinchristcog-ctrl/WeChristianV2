  ...
  try {
    ...
    https.get(options, (res) => {
      ...
      res.on('end', () => {
        ...
        const search = ['Have_You_Baptized__c', 'Date_of_Baptism__c', 'Church_Of_Baptism__c', 'Church_Events__c'];
        const fields = data.fields
          .filter(f => search.includes(f.name))
          .map(f => ({ 
            name: f.name, 
            label: f.label, 
            type: f.type,
            picklistValues: f.picklistValues ? f.picklistValues.map(pv => pv.value) : null
          }));
